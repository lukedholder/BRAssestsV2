...
public class EnemyController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // add an offset of +2 on the enemy's y.position to make up
        // for the fact that they go after batmans feet
        if (batman.transform.position.y + 1.5f > gameObject.transform.position.y) {
            enemyBehaviourObject.Move(gameObject, "down");
        } else if (batman.transform.position.y + 2.5f < gameObject.transform.position.y) {
