...
public class UserDB : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // AddUser("bob", "456", "user", highscore: 1000);
    ...
    public bool UserIsAdmin(int uid)
    {
        string role = "";
        using (var conn = new SqliteConnection(dbName))
        {
            ...
            using (var command = conn.CreateCommand())
            {
                // get the role from the user with the given uid
                command.CommandText = @"SELECT role FROM users WHERE uid = @param1 LIMIT 1;";
                command.Parameters.AddWithValue("@param1", uid);
